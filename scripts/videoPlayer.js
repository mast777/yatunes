export const videoPlayerInit = () => {
    ...
    videoPlayer.addEventListener('timeupdate', () => {
        ...
        let miutePassed = Math.floor(currentTime / 60);
        let secondsPassed = Math.floor(currentTime % 60);

        let miuteTotal = Math.floor(duration / 60);
        let secondsTotal = Math.floor(duration % 60);

        videoTimePassed.textContent = addZero(miutePassed)+':'+addZero(secondsPassed);
        videoTimeTotal.textContent = addZero(miuteTotal)+':'+addZero(secondsTotal);
